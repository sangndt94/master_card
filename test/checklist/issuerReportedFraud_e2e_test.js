...
const groupRejectedCharts = [
    ["Transactions by Month", "aacGSwlpbrvX"],
    ["Outstanding Rejects", "aaw7EMVxeQFj"],
    ["Transaction Details", "aapO7E2DiCle"]
];

const groupSuspendedCharts = [
    ["Transactions by Month", "aacGSwlpbrvX"],
    ["Transactions by Suspend Reason", "aevgOJxKg87K"],
    ["Outstanding Suspends", "aaw7EMVxeQFj"],
    ["Transaction Details", "aapO7E2DiCle"],
    ["Transactions by Fraud Type", "aaNqkBBAbPwf"]
];

const groupErroredAndDeletedCharts = [
    ["Transactions by Month", "aacGSwlpbrvX"],
    ["Transaction Details", "aapO7E2DiCle"]
];
...
fixtureWithLogin("Reported Fraud Issuer", 1, "reported-fraud-issuer/successful");
...
test(`should apply filter and render correctly at REJECTED TRANSACTIONS`, async () =>{
    await navigateTo("reported-fraud-issuer/rejected");
    const visualizationIDs = ["aacGSwlpbrvX", "aaqGm6qHe1jd", "aa6Gytdhh3sW", "abv6YJWzbXqc",
        "aaw7EMVxeQFj", "aapO7E2DiCle"];
    const visualizationID = visualizationIDs[0];
    await assertActiveLeftMenuItem("REJECTED TRANSACTIONS");
    await waitForLoading( await getVisualizationByIdentifier(visualizationID));
    await assertDefaultValueAllFilters([enteredDate, errorCode, submissionType]);
    await assertAllVisualizationsExist(visualizationIDs);
    await checkRenderChartByTitle(groupRejectedCharts);
});

test(`should apply filter and render correctly at SUSPENDED TRANSACTIONS`, async () =>{
    await navigateTo("reported-fraud-issuer/suspended");
    const visualizationIDs = ["aacGSwlpbrvX", "aaqGm6qHe1jd", "aa6Gytdhh3sW", "abv6YJWzbXqc",
        "aevgOJxKg87K", "aaw7EMVxeQFj", "aapO7E2DiCle"];
    const visualizationID = visualizationIDs[0];
    await assertActiveLeftMenuItem("SUSPENDED TRANSACTIONS");
    await waitForLoading( await getVisualizationByIdentifier(visualizationID));
    await assertDefaultValueAllFilters([enteredDate, suspendReason]);
    await assertAllVisualizationsExist(visualizationIDs);
    await checkRenderChartByTitle(groupSuspendedCharts);
});

test(`should apply filter and render correctly at ERRORED AND DELETED TRANSACTIONS`, async () =>{
    await navigateTo("reported-fraud-issuer/errored-deleted");
    const visualizationIDs = ["aacGSwlpbrvX", "aaUJltaacheK", "aapO7E2DiCle"];
    const visualizationID = visualizationIDs[0];
    await assertActiveLeftMenuItem("ERRORED AND DELETED TRANSACTIONS");
    await waitForLoading( await getVisualizationByIdentifier(visualizationID));
    await assertDefaultValueAllFilters([enteredDate, errorCode, submissionType]);
    await assertAllVisualizationsExist(visualizationIDs);
    await checkRenderChartByTitle(groupErroredAndDeletedCharts);
});

test(`should apply filter and render correctly at GEOGRAPHY`, async () =>{
    await navigateTo("reported-fraud-issuer/geography");
...
test(`should apply filter and render correctly at POS`, async () =>{
    await navigateTo("reported-fraud-issuer/pos");
